...
export async function POST(request: NextRequest) {
  try {
    ...
    if (guestError) {
      return jsonError(`Guest reset failed: ${guestError.message}`, 500);
    }
    ...
    if (orderError) {
      return jsonError(`Order reset failed: ${orderError.message}`, 500);
    }

    // Print jobs are not part of the customer bill, but cancelling pending jobs avoids old tickets
    // printing after the table has been turned over. Ignore this if the print table is not installed yet.
